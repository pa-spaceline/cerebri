...
LOG_MODULE_REGISTER(hab_fsm, CONFIG_CEREBRI_HAB_LOG_LEVEL);
...
static void hab_fsm_init(struct context *ctx)
{
	zros_node_init(&ctx->node, "hab_fsm");
...
static void hab_fsm_fini(struct context *ctx)
{
...
static void fsm_compute_input(struct status_input *input, const struct context *ctx)
{
	...
	if (input->usb_connected) {
		...
		input->fuel_low = ctx->battery_state.voltage <
				  CONFIG_CEREBRI_HAB_BATTERY_NCELLS *
					  CONFIG_CEREBRI_HAB_BATTERY_CELL_LOW_MILLIVOLT / 1000.0;
		input->fuel_critical = ctx->battery_state.voltage <
				       CONFIG_CEREBRI_HAB_BATTERY_NCELLS *
					       CONFIG_CEREBRI_HAB_BATTERY_CELL_MIN_MILLIVOLT /
					       1000.0;
	}
#else
	input->fuel_low = false;
...
static void fsm_update(synapse_pb_Status *status, const struct status_input *input)
{
	...
	transition(&status->mode,                                          // state
		   ...
		   &status->request_seq, &status->request_rejected,        // request
		   // guards
		   1, status->arming == synapse_pb_Status_Arming_ARMING_ARMED, "disarm required");
				  ...
				  const struct context *ctx)
{
	...
	double bat_max = CONFIG_CEREBRI_HAB_BATTERY_NCELLS *
			 CONFIG_CEREBRI_HAB_BATTERY_CELL_MAX_MILLIVOLT / 1000.0;
	double bat_min = CONFIG_CEREBRI_HAB_BATTERY_NCELLS *
			 CONFIG_CEREBRI_HAB_BATTERY_CELL_MIN_MILLIVOLT / 1000.0;
...
static void hab_fsm_run(void *p0, void *p1, void *p2)
{
...
static int start(struct context *ctx)
{
	k_tid_t tid = k_thread_create(&ctx->thread_data, ctx->stack_area, ctx->stack_size,
				      hab_fsm_run, ctx, NULL, NULL, MY_PRIORITY, 0, K_FOREVER);
	k_thread_name_set(tid, "hab_fsm");
	k_thread_start(tid);
	return 0;
}

static int hab_fsm_cmd_handler(const struct shell *sh, size_t argc, char **argv, void *data)
{
	ARG_UNUSED(argc);
...
SHELL_SUBCMD_DICT_SET_CREATE(sub_hab_fsm, hab_fsm_cmd_handler, (start, &g_ctx, "start"),
			     (stop, &g_ctx, "stop"), (status, &g_ctx, "status"));

SHELL_CMD_REGISTER(hab_fsm, &sub_hab_fsm, "hab fsm commands", NULL);

static int hab_fsm_sys_init(void)
{
	return start(&g_ctx);
};

SYS_INIT(hab_fsm_sys_init, APPLICATION, 1);
