...
#include "app/hab/casadi/hab_loglinear.h"
...
CEREBRI_NODE_LOG_INIT(hab_attitude, LOG_LEVEL_WRN);
...
static void hab_attitude_init(struct context *ctx)
{
	zros_node_init(&ctx->node, "hab_attiude");
...
static void hab_attitude_fini(struct context *ctx)
{
...
static void hab_attitude_run(void *p0, void *p1, void *p2)
{
	struct context *ctx = p0;
	ARG_UNUSED(p1);
	ARG_UNUSED(p2);

	hab_attitude_init(ctx);
	...
	static const double kp[3] = {
		CONFIG_CEREBRI_HAB_ROLL_KP * 1e-6,
		CONFIG_CEREBRI_HAB_PITCH_KP * 1e-6,
		CONFIG_CEREBRI_HAB_YAW_KP * 1e-6,
	};
...
static int start(struct context *ctx)
{
	k_tid_t tid =
		k_thread_create(&ctx->thread_data, ctx->stack_area, ctx->stack_size,
				hab_attitude_run, ctx, NULL, NULL, MY_PRIORITY, 0, K_FOREVER);
	k_thread_name_set(tid, "hab_attitude");
	k_thread_start(tid);
	return 0;
}

static int hab_attitude_cmd_handler(const struct shell *sh, size_t argc, char **argv, void *data)
{
	ARG_UNUSED(argc);
...
SHELL_SUBCMD_DICT_SET_CREATE(sub_hab_attitude, hab_attitude_cmd_handler, (start, &g_ctx, "start"),
			     (stop, &g_ctx, "stop"), (status, &g_ctx, "status"));

SHELL_CMD_REGISTER(hab_attitude, &sub_hab_attitude, "hab attitude commands", NULL);

static int hab_attitude_sys_init(void)
{
	return start(&g_ctx);
};

SYS_INIT(hab_attitude_sys_init, APPLICATION, 99);
