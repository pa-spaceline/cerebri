...
CEREBRI_NODE_LOG_INIT(hab_command, LOG_LEVEL_WRN);
...
static struct context g_ctx = {
	...
	.q = {1, 0, 0, 0},
	.dt = 0,
	.thrust_trim = CONFIG_CEREBRI_HAB_THRUST_TRIM * 1e-3,
	.thrust_delta = CONFIG_CEREBRI_HAB_THRUST_DELTA * 1e-3,
};

static void hab_command_init(struct context *ctx)
{
	zros_node_init(&ctx->node, "hab_command");
...
static void hab_command_fini(struct context *ctx)
{
...
static void hab_command_run(void *p0, void *p1, void *p2)
{
	struct context *ctx = p0;
	ARG_UNUSED(p1);
	ARG_UNUSED(p2);

	hab_command_init(ctx);
	...
	hab_command_fini(ctx);
}

static int start(struct context *ctx)
{
	k_tid_t tid = k_thread_create(&ctx->thread_data, ctx->stack_area, ctx->stack_size,
				      hab_command_run, ctx, NULL, NULL, MY_PRIORITY, 0, K_FOREVER);
	k_thread_name_set(tid, "hab_command");
	k_thread_start(tid);
	return 0;
}

static int hab_command_cmd_handler(const struct shell *sh, size_t argc, char **argv, void *data)
{
	ARG_UNUSED(argc);
...
SHELL_SUBCMD_DICT_SET_CREATE(sub_hab_command, hab_command_cmd_handler, (start, &g_ctx, "start"),
			     (stop, &g_ctx, "stop"), (status, &g_ctx, "status"));

SHELL_CMD_REGISTER(hab_command, &sub_hab_command, "hab command arguments", NULL);

static int hab_command_sys_init(void)
{
	return start(&g_ctx);
};

SYS_INIT(hab_command_sys_init, APPLICATION, 99);
