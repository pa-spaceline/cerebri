...
CEREBRI_NODE_LOG_INIT(hab_allocation, LOG_LEVEL_WRN);
...
static void hab_allocation_init(struct context *ctx)
{
	zros_node_init(&ctx->node, "hab_allocation");
...
static void hab_allocation_fini(struct context *ctx)
{
...
static void hab_allocation_run(void *p0, void *p1, void *p2)
{
	struct context *ctx = p0;
	ARG_UNUSED(p1);
	ARG_UNUSED(p2);

	hab_allocation_init(ctx);
	...
	while (k_sem_take(&ctx->running, K_NO_WAIT) < 0) {
		...
		if (rc < 0) {
			...
			static double const l = CONFIG_CEREBRI_HAB_MOTOR_L_MM * 1e-3;
			static double const Cm = CONFIG_CEREBRI_HAB_MOTOR_CM * 1e-6;
			static double const Ct = CONFIG_CEREBRI_HAB_MOTOR_CT * 1e-9;
			double omega[4];
...
static int start(struct context *ctx)
{
	k_tid_t tid =
		k_thread_create(&ctx->thread_data, ctx->stack_area, ctx->stack_size,
				hab_allocation_run, ctx, NULL, NULL, MY_PRIORITY, 0, K_FOREVER);
	k_thread_name_set(tid, "hab_allocation");
	k_thread_start(tid);
	return 0;
}

static int hab_allocation_cmd_handler(const struct shell *sh, size_t argc, char **argv, void *data)
{
	ARG_UNUSED(argc);
...
SHELL_SUBCMD_DICT_SET_CREATE(sub_hab_allocation, hab_allocation_cmd_handler,
			     (start, &g_ctx, "start"), (stop, &g_ctx, "stop"),
			     (status, &g_ctx, "status"));

SHELL_CMD_REGISTER(hab_allocation, &sub_hab_allocation, "hab allocation commands", NULL);

static int hab_allocation_sys_init(void)
{
	return start(&g_ctx);
};

SYS_INIT(hab_allocation_sys_init, APPLICATION, 99);
