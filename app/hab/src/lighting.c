...
CEREBRI_NODE_LOG_INIT(hab_lighting, LOG_LEVEL_WRN);
...
static void hab_lighting_init(struct context *ctx)
{
	zros_node_init(&ctx->node, "hab_lighting");
...
static void hab_lighting_fini(struct context *ctx)
{
...
static void hab_lighting_run(void *p0, void *p1, void *p2)
{
	struct context *ctx = p0;
	ARG_UNUSED(p1);
	ARG_UNUSED(p2);

	hab_lighting_init(ctx);
...
static int start(struct context *ctx)
{
	k_tid_t tid =
		k_thread_create(&ctx->thread_data, ctx->stack_area, ctx->stack_size,
				hab_lighting_run, ctx, NULL, NULL, MY_PRIORITY, 0, K_FOREVER);
	k_thread_name_set(tid, "hab_lighting");
	k_thread_start(tid);
	return 0;
}

static int hab_lighting_cmd_handler(const struct shell *sh, size_t argc, char **argv, void *data)
{
	ARG_UNUSED(argc);
...
SHELL_SUBCMD_DICT_SET_CREATE(sub_hab_lighting, hab_lighting_cmd_handler, (start, &g_ctx, "start"),
			     (stop, &g_ctx, "stop"), (status, &g_ctx, "status"));

SHELL_CMD_REGISTER(hab_lighting, &sub_hab_lighting, "hab lighting commands", NULL);

static int hab_lighting_sys_init(void)
{
	return start(&g_ctx);
};

SYS_INIT(hab_lighting_sys_init, APPLICATION, 10);
