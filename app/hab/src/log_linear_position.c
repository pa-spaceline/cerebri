...
CEREBRI_NODE_LOG_INIT(hab_position, LOG_LEVEL_WRN);

static K_THREAD_STACK_DEFINE(g_my_stack_area, MY_STACK_SIZE);

static const double thrust_trim = CONFIG_CEREBRI_HAB_THRUST_TRIM * 1e-3;
...
static void hab_position_init(struct context *ctx)
{
	zros_node_init(&ctx->node, "hab_position");
...
static void hab_position_fini(struct context *ctx)
{
...
static void hab_position_run(void *p0, void *p1, void *p2)
{
	struct context *ctx = p0;
	ARG_UNUSED(p1);
	ARG_UNUSED(p2);

	hab_position_init(ctx);
...
static int start(struct context *ctx)
{
	k_tid_t tid =
		k_thread_create(&ctx->thread_data, ctx->stack_area, ctx->stack_size,
				hab_position_run, ctx, NULL, NULL, MY_PRIORITY, 0, K_FOREVER);
	k_thread_name_set(tid, "hab_position");
	k_thread_start(tid);
	return 0;
}

static int hab_position_cmd_handler(const struct shell *sh, size_t argc, char **argv, void *data)
{
	ARG_UNUSED(argc);
...
SHELL_SUBCMD_DICT_SET_CREATE(sub_hab_position, hab_position_cmd_handler, (start, &g_ctx, "start"),
			     (stop, &g_ctx, "stop"), (status, &g_ctx, "status"));

SHELL_CMD_REGISTER(hab_position, &sub_hab_position, "hab position commands", NULL);

static int hab_position_sys_init(void)
{
	return start(&g_ctx);
};

SYS_INIT(hab_position_sys_init, APPLICATION, 99);
