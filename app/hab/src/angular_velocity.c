...
CEREBRI_NODE_LOG_INIT(hab_angular_velocity, LOG_LEVEL_WRN);
...
static void hab_angular_velocity_init(struct context *ctx)
{
	zros_node_init(&ctx->node, "hab_angular_velocity");
...
static void hab_angular_velocity_fini(struct context *ctx)
{
...
static void hab_angular_velocity_run(void *p0, void *p1, void *p2)
{
	struct context *ctx = p0;
	ARG_UNUSED(p1);
	ARG_UNUSED(p2);

	hab_angular_velocity_init(ctx);
	...
	static const double kp[3] = {
		CONFIG_CEREBRI_HAB_ROLLRATE_KP * 1e-6 * 20,
		CONFIG_CEREBRI_HAB_PITCHRATE_KP * 1e-6 * 20,
		CONFIG_CEREBRI_HAB_YAWRATE_KP * 1e-6 * 20,
	};

	static const double ki[3] = {
		CONFIG_CEREBRI_HAB_ROLLRATE_KI * 1e-6 * 20,
		CONFIG_CEREBRI_HAB_PITCHRATE_KI * 1e-6 * 20,
		CONFIG_CEREBRI_HAB_YAWRATE_KI * 1e-6 * 20,
	};

	static const double kd[3] = {
		CONFIG_CEREBRI_HAB_ROLLRATE_KD * 1e-6 * 20,
		CONFIG_CEREBRI_HAB_PITCHRATE_KD * 1e-6 * 20,
		CONFIG_CEREBRI_HAB_YAWRATE_KD * 1e-6 * 20,
	};

	static const double i_max[3] = {
		CONFIG_CEREBRI_HAB_ROLLRATE_IMAX * 1e-6,
		CONFIG_CEREBRI_HAB_PITCHRATE_IMAX * 1e-6,
		CONFIG_CEREBRI_HAB_YAWRATE_IMAX * 1e-6,
	};

	static const double f_cut = CONFIG_CEREBRI_HAB_ATTITUDE_RATE_FCUT * 1e-3;
	...
	hab_angular_velocity_fini(ctx);
}

static int start(struct context *ctx)
{
	k_tid_t tid = k_thread_create(&ctx->thread_data, ctx->stack_area, ctx->stack_size,
				      hab_angular_velocity_run, ctx, NULL, NULL, MY_PRIORITY, 0,
				      K_FOREVER);
	k_thread_name_set(tid, "hab_angular_velocity");
	k_thread_start(tid);
	return 0;
}

static int hab_angular_velocity_cmd_handler(const struct shell *sh, size_t argc, char **argv,
					     void *data)
{
	ARG_UNUSED(argc);
...
SHELL_SUBCMD_DICT_SET_CREATE(sub_hab_angular_velocity, hab_angular_velocity_cmd_handler,
			     (start, &g_ctx, "start"), (stop, &g_ctx, "stop"),
			     (status, &g_ctx, "status"));

SHELL_CMD_REGISTER(hab_angular_velocity, &sub_hab_angular_velocity,
		   "hab angular velocity commands", NULL);

static int hab_angular_velocity_sys_init(void)
{
	return start(&g_ctx);
};

SYS_INIT(hab_angular_velocity_sys_init, APPLICATION, 99);
